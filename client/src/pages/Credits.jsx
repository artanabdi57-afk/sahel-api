...
import { 
  ...
  CreditCard, Phone, Plus, Search, X, Calendar, Wallet, History, ArrowUpRight 
} from "lucide-react";
...
export default function Credits() {
  ...
  // Data processing logic (Simplified for clarity)
  const customers = useMemo(() => {
    const groups = new Map();
    credits.forEach(c => {
      const key = getCustomerKey(c);
      if (!groups.has(key)) {
        groups.set(key, { ...c, key, items: [], total_owed: 0, total_original: 0, total_paid: 0, has_overdue: false });
      }
  ...
  return (
    <div className={`space-y-6 pb-20 font-sans min-h-screen p-4 md:p-8 bg-[#FAF9F6]`} dir={isRTL ? 'rtl' : 'ltr'}>
      
      {/* 1. HEADER & SEARCH */}
      <header className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-black text-blue-900 tracking-tight">{t("credits") || "Customer Credits"}</h1>
            <p className="text-slate-500 font-medium text-sm">{t("creditSubtext") || "Manage debts and incoming payments"}</p>
          </div>
          <button 
          ...
          >
            <Plus size={20}/> {t("giveMoney") || "Record Debt"}
          </button>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <SummaryCard label={t("totalOwed") || "Total Owed"} value={grandTotalOwed} icon={Wallet} colorClass="bg-blue-50 text-blue-600" />
          <SummaryCard label={t("overdue") || "Overdue"} value={customers.filter(c=>c.has_overdue).reduce((s,c)=>s+c.total_owed,0)} icon={AlertTriangle} colorClass="bg-orange-50 text-orange-600" />
          ...
          <div className="md:col-span-2 flex items-center gap-2 bg-white p-2 rounded-[1.5rem] border border-slate-200/60 shadow-sm">
            <div className="relative flex-1">
              ...
              <input 
                className={`w-full bg-slate-50 border-none rounded-xl py-2 ${isRTL?'pr-10 pl-4':'pl-10 pr-4'} text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500`}
                placeholder={t("searchCustomer") || "Search name or phone..."}
                value={search}
      ...
      <section className="space-y-4">
        {customers.length === 0 ? (
          <div className="bg-white rounded-[2rem] p-10 text-center border border-slate-100 shadow-sm">
            <EmptyState title={t("noCredits") || "No records found"} />
          </div>
        ) : (
          customers.map(customer => {
            const isOpen = openCustomer === customer.key;
            return (
              <div key={customer.key} className={`bg-white border rounded-[2rem] overflow-hidden transition-all ${customer.has_overdue ? 'border-orange-200 shadow-orange-100/20 shadow-xl' : 'border-slate-100 shadow-sm hover:shadow-md'}`}>
                {/* Customer Row Header */}
                <button 
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        {customer.has_overdue && <span className="bg-orange-100 text-orange-700 text-[9px] font-black px-2 py-1 rounded-full uppercase tracking-tighter">{t("overdue") || "Overdue"}</span>}
                      </div>
                      <p className="text-slate-400 text-xs font-bold flex items-center gap-1 mt-0.5"><Phone size={12}/> {customer.customer_phone}</p>
                    </div>
                  </div>

                  <div className="flex items-center gap-8">
                    <div className="text-right">
                       <p className="text-[10px] font-black text-slate-400 uppercase">{t("remaining") || "Remaining"}</p>
                ...
                {/* Expanded Details */}
                {isOpen && (
                  <div className="p-6 bg-slate-50/50 border-t border-slate-100 animate-in fade-in slide-in-from-top-2">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      
                      {/* Left: Itemized Debt */}
                      <div className="lg:col-span-2 space-y-3">
                        <h4 className="text-xs font-black text-slate-400 uppercase mb-4 flex items-center gap-2"><History size={14}/> {t("debtHistory") || "Debt History"}</h4>
                        {customer.items.map((item, idx) => (
                          <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200/60 shadow-sm flex justify-between items-center">
                            <div>
                               ...
                               <p className="text-[10px] font-bold text-slate-400">{formatDT(item.created_at, language)}</p>
                            </div>
                            <div className="text-right">
                      ...
                      <div className="bg-blue-900 text-white rounded-[2rem] p-6 shadow-xl shadow-blue-200">
                        <div className="flex justify-between items-center mb-6">
                           <h4 className="text-xs font-black uppercase tracking-widest text-blue-200">{t("payment") || "Payment"}</h4>
                           <ArrowUpRight size={16} className="text-blue-300" />
                        </div>
                        <div className="mb-8">
                           <p className="text-3xl font-black mb-1">{formatMoney(customer.total_owed)}</p>
                           <p className="text-[10px] font-bold text-blue-300 uppercase">{t("totalBalance") || "Total Balance Due"}</p>
                        </div>
                        <div className="space-y-3">
                          ...
                          >
                            <Check size={16} className="inline mr-2" /> {t("markPaid") || "Mark Fully Paid"}
                          </button>
                        </div>
                      </div>
      ...
      {/* Global Total Bar */}
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-4xl bg-blue-900 text-white p-4 rounded-[2rem] shadow-2xl flex justify-between items-center z-40 border border-white/10 backdrop-blur-md">
         <div className="flex items-center gap-3">
           <div className="p-2 bg-white/10 rounded-xl"><CreditCard size={18}/></div>
           <span className="text-xs font-black uppercase tracking-widest">{t("grandTotal") || "Total Outstanding"}</span>
         </div>
