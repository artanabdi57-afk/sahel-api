...
function ProductSearch({ products, value, onSelect }) {
  ...
  const filtered = query.trim()
    ? products.filter((p) => p.name.toLowerCase().includes(query.trim().toLowerCase())).slice(0, 10)
    : products.slice(0, 10);
    ...
    <div ref={ref} className="relative">
      {selected ? (
        <div className="field flex items-center justify-between gap-2">
          <span className="text-sm font-medium text-slate-800 truncate">{selected.name}</span>
          <button type="button" onClick={handleClear} className="text-slate-400 hover:text-slate-700 flex-shrink-0">
            <X className="h-4 w-4" />
          </button>
        </div>
      ) : (
      ...
      {open && !selected ? (
        <div className="absolute left-0 right-0 top-full z-50 mt-1 max-h-64 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-[0_18px_45px_rgba(15,23,42,0.14)]">
          {filtered.length === 0 ? (
            ...
            filtered.map((p) => (
              ...
              >
                ...
                <span className="text-xs text-slate-400">{p.quantity} {p.unit || "pcs"} in stock</span>
              </button>
            ))
          )}
        </div>
