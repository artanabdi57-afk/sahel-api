...
import {
  ...
  Users, X, XCircle, Zap, MapPin, AlertOctagon
} from "lucide-react";
...
// Tables to cascade-delete when removing a shop
const SHOP_CHILD_TABLES = ["credit_payments", "credits", "sales", "purchase_orders", "expenses", "products"];
...
async function sbRest(table, params = "") {
  const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}?${params}`, { headers: ADMIN_HEADERS });
...
// Hard-delete shop + all child data + user account
async function hardDeleteShop(shop) {
  // 1. Delete child records
  for (const table of SHOP_CHILD_TABLES) {
    await sbDelete(table, `shop_id=eq.${shop.id}`);
  }
  // 2. Delete shop
  await sbDelete("shops", `id=eq.${shop.id}`);
  // 3. Delete user account
  if (shop.owner_id) {
    await sbDelete("users", `id=eq.${shop.owner_id}`);
  }
}
...
function Btn({ children, onClick, variant = "secondary", size = "md", disabled, className = "" }) {
  ...
  const variants = {
    ...
    dangerOutline: "bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100",
...
function ConfirmDialog({ title, message, confirmLabel = "Confirm", danger = true, onConfirm, onCancel }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
      <div className="w-full max-w-sm rounded-3xl border border-slate-200 bg-white shadow-2xl p-6">
        <div className={`mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl ${danger ? "bg-rose-100" : "bg-blue-100"}`}>
          <AlertOctagon className={`h-7 w-7 ${danger ? "text-rose-600" : "text-blue-600"}`} />
        </div>
        <h3 className="text-center text-lg font-black text-slate-950">{title}</h3>
        <p className="mt-2 text-center text-sm font-semibold text-slate-500">{message}</p>
        <div className="mt-6 flex gap-3">
          <button onClick={onCancel} className="flex-1 rounded-xl border border-slate-200 bg-white py-2.5 text-sm font-black text-slate-700 hover:bg-slate-50 transition-all">
            Cancel
          </button>
          <button onClick={onConfirm} className={`flex-1 rounded-xl py-2.5 text-sm font-black text-white transition-all shadow-sm ${danger ? "bg-rose-600 hover:bg-rose-700" : "bg-blue-600 hover:bg-blue-700"}`}>
            {confirmLabel}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-6">
        ...
        <StatCard label="Est. MRR" value={fmtUSD(mrr)} icon={CreditCard} color="green" sub="monthly" />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4">Recent Admin Actions</p>
          <div className="space-y-3">
...
function UsersView({ shops, setShops, addLog, toast, role, reloadShops }) {
  ...
  const [confirm, setConfirm] = useState(null); // { shop, action }
  const PER = 10;
  ...
  async function doSuspend(s) {
    try {
      await sbPatch("shops", `id=eq.${s.id}`, { status: "suspended" });
      await sbPatch("users", `id=eq.${s.owner_id}`, { status: "suspended" });
      setShops(p => p.map(x => x.id === s.id ? { ...x, status: "suspended" } : x));
      addLog({ action: "Suspended account", target: s.owner_name });
      toast(`${s.shop_name} suspended.`);
  ...
  async function doUnsuspend(s) {
    try {
      await sbPatch("shops", `id=eq.${s.id}`, { status: "active" });
      await sbPatch("users", `id=eq.${s.owner_id}`, { status: "active" });
      setShops(p => p.map(x => x.id === s.id ? { ...x, status: "active" } : x));
      addLog({ action: "Reactivated account", target: s.owner_name });
      toast(`${s.shop_name} reactivated.`);
    } catch (e) { toast("Failed: " + e.message, "error"); }
  }

  // Hard delete — permanently removes shop + all data + user
  async function doHardDelete(s) {
    setSaving(true);
    try {
      await hardDeleteShop(s);
      setShops(p => p.filter(x => x.id !== s.id));
      addLog({ action: "PERMANENTLY DELETED shop + all data + user", target: s.owner_name });
      toast(`${s.shop_name} and all its data have been permanently deleted.`);
      setModal(null);
      setConfirm(null);
  ...
  async function doSaveEdit() {
    ...
    setSaving(true);
    try {
      await sbPatch("shops", `id=eq.${s.id}`, {
  ...
  async function doApplyPlan() {
    ...
    setSaving(true);
    try {
      await sbPatch("shops", `id=eq.${s.id}`, { plan: planForm.plan, plan_expires_at: expiry });
  ...
  // Trigger confirm dialog for delete
  function askDelete(shop) {
    setConfirm({ shop, action: "delete" });
  }
    ...
    <div className="space-y-4">
      ...
      {confirm?.action === "delete" && (
        <ConfirmDialog
          ...
          message={`This will delete "${confirm.shop.shop_name}", all its products, sales, expenses, credits, and the owner's account. This cannot be undone.`}
          confirmLabel="Yes, Delete Everything"
          danger
          onConfirm={() => doHardDelete(confirm.shop)}
                  ...
                  <tr key={s.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <Btn size="sm" variant="ghost" onClick={() => openView(s)} title="View details"><Eye className="h-3.5 w-3.5" /></Btn>
                        {canEdit && <>
                          <Btn size="sm" variant="ghost" onClick={() => openEdit(s)} title="Edit info"><User className="h-3.5 w-3.5" /></Btn>
                          <Btn size="sm" variant="ghost" onClick={() => openPlan(s)} title="Manage plan"><Star className="h-3.5 w-3.5" /></Btn>
                          {s.status === "active"
                            ? <Btn size="sm" variant="dangerOutline" onClick={() => doSuspend(s)} title="Suspend"><ShieldOff className="h-3.5 w-3.5" /></Btn>
                            : <Btn size="sm" variant="success" onClick={() => doUnsuspend(s)} title="Reactivate"><ShieldCheck className="h-3.5 w-3.5" /></Btn>
                          }
                          <Btn size="sm" variant="danger" onClick={() => askDelete(s)} title="Permanently delete"><Trash2 className="h-3.5 w-3.5" /></Btn>
                        </>}
                      </div>
                    </td>
                  </tr>
      ...
      {pages > 1 && (
        <div className="flex items-center justify-between">
          <p className="text-xs font-bold text-slate-400">Page {page} of {pages} ({filtered.length} total)</p>
        ...
        <Modal title={modal.shop.shop_name} onClose={() => setModal(null)} width="max-w-2xl">
          ...
          {canEdit && (
            <div className="mt-5 flex flex-wrap gap-2 border-t border-slate-100 pt-5">
              ...
              {modal.shop.status === "active"
                ? <Btn size="sm" variant="dangerOutline" onClick={() => { doSuspend(modal.shop); setModal(null); }}><ShieldOff className="h-3.5 w-3.5" />Suspend</Btn>
                : <Btn size="sm" variant="success" onClick={() => { doUnsuspend(modal.shop); setModal(null); }}><ShieldCheck className="h-3.5 w-3.5" />Reactivate</Btn>
              }
              <Btn size="sm" variant="danger" onClick={() => { setModal(null); setTimeout(() => askDelete(modal.shop), 50); }}><Trash2 className="h-3.5 w-3.5" />Delete Account</Btn>
            </div>
          )}
        </Modal>
      )}
...
function SubscriptionsView({ shops, setShops, addLog, toast }) {
  ...
  async function extend(s, days) {
    ...
    try {
      await sbPatch("shops", `id=eq.${s.id}`, { plan: "paid", plan_expires_at: expiry });
  ...
  async function cancel(s) {
    try {
      await sbPatch("shops", `id=eq.${s.id}`, { plan: "free", plan_expires_at: null });
    ...
    <div className="space-y-6">
      {confirm && (
        <ConfirmDialog
          ...
          message={`Downgrade "${confirm.shop_name}" to the Free plan?`}
          confirmLabel="Yes, Cancel"
          danger
          onConfirm={() => { cancel(confirm); setConfirm(null); }}
          onCancel={() => setConfirm(null)}
        />
      )}
                ...
                <tr key={s.id} className="hover:bg-slate-50/60">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-1.5 flex-wrap">
                      ...
                      <Btn size="sm" onClick={() => extend(s, 365)}>+1yr</Btn>
                      {s.plan === "paid" && <Btn size="sm" variant="dangerOutline" onClick={() => setConfirm(s)}><X className="h-3 w-3" />Cancel</Btn>}
                    </div>
                  </td>
                </tr>
              );
...
function AnalyticsView({ shops }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-x-auto">
        <div className="border-b border-slate-100 p-4">
          <h2 className="font-black text-slate-950">All Clients — Full Overview</h2>
        </div>
        <table className="min-w-full divide-y divide-slate-100 text-sm">
          <thead className="bg-slate-50">
            <tr>
          ...
          <tbody className="divide-y divide-slate-100">
            {shops.map(s => (
              <tr key={s.id} className="hover:bg-slate-50/60">
                ...
                <td className="px-4 py-3 whitespace-nowrap">
                  {s.phone ? <span className="font-semibold text-slate-700">{s.phone}</span> : <Badge color="amber">Missing</Badge>}
                </td>
                <td className="px-4 py-3 whitespace-nowrap">
                  {s.location ? <span className="font-semibold text-slate-700">{s.location}</span> : <Badge color="amber">Missing</Badge>}
                </td>
            ...
            {logs.length === 0
              ...
              : logs.map((l, i) => (
                <tr key={l.id || i} className="hover:bg-slate-50/60">
                  ...
                  <td className="px-4 py-3"><Badge color={l.action.includes("DELETE") || l.action.includes("delete") ? "red" : "blue"}>{l.target}</Badge></td>
                </tr>
              ))
            }
          </tbody>
