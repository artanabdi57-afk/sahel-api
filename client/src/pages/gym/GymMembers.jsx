...
export default function GymMembers() {
  return (
    <EntityCrud
      ...
      emptyDescription="Add your first gym member to start tracking them."
      fields={[
      ...
      columns={[
        ...
        { key: "status", label: "Status", render: (r) => <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${r.status === "active" ? "bg-green-50 text-green-700" : "bg-slate-100 text-slate-500"}`}>{r.status || "active"}</span> },
      ]}
    />
  );
}
