...
export default function GymCheckins() {
  ...
  const [form, setForm] = useState({ name: "", gender: "", phone: "", plan: "monthly", paymentAmount: "", paymentDate: today });
  const [status, setStatus] = useState({ loading: true, saving: false, error: "", success: "" });
  async function load() { setStatus((s) => ({ ...s, loading: true, error: "" })); try { const r = await apiRequest("/gym/members"); setMembers(r.data || []); } catch (e) { setStatus((s) => ({ ...s, error: e.message })); } finally { setStatus((s) => ({ ...s, loading: false })); } }
  useEffect(() => { load(); }, []);
  function update(field, value) { setForm((f) => ({ ...f, [field]: value })); }
  async function addMember(e) {
    e.preventDefault(); if (!form.name.trim() || !form.gender || !form.phone.trim() || !form.paymentAmount || !form.paymentDate) return;
    const paidUntil = addDays(form.paymentDate, form.plan === "weekly" ? 7 : 30);
    setStatus((s) => ({ ...s, saving: true, error: "", success: "" }));
    try {
      const r = await apiRequest("/gym/members", { method: "POST", body: JSON.stringify({ name: form.name.trim(), gender: form.gender, phone: form.phone.trim(), registration_paid_amount: Number(form.paymentAmount), registration_paid_until: paidUntil, payment_for: form.plan === "weekly" ? "Weekly Membership" : "Monthly Membership", payment_date: form.paymentDate, membership_plan: form.plan }) });
      setForm({ name: "", gender: "", phone: "", plan: "monthly", paymentAmount: "", paymentDate: today });
      setStatus((s) => ({ ...s, success: `${r.data?.name || "New member"} registered. Next payment is ${new Date(`${paidUntil}T00:00:00`).toLocaleDateString()}.` })); await load();
    } catch (e) { setStatus((s) => ({ ...s, error: e.message })); } finally { setStatus((s) => ({ ...s, saving: false })); }
  }
  if (status.loading) return <LoadingState />;
  const nextPayment = addDays(form.paymentDate, form.plan === "weekly" ? 7 : 30);
  return <div className="space-y-6 motion-card">
    <div className="rounded-3xl border border-emerald-100 bg-gradient-to-r from-white via-emerald-50 to-blue-50 p-6"><p className="text-xs font-bold uppercase tracking-[0.18em] text-emerald-600">Member registration</p><h1 className="mt-1 text-3xl font-black">Add New Member</h1><p className="mt-2 max-w-3xl text-sm text-slate-500">Register the member and record the first payment. The next payment date is calculated automatically.</p></div>
    ...
    <div className="grid gap-6 lg:grid-cols-[560px_1fr]">
      <form onSubmit={addMember} className="panel p-6 shadow-sm">
        ...
        <div className="my-6 border-t border-slate-100" /><div className="mb-4 flex items-center gap-2"><CreditCard className="h-5 w-5 text-emerald-600" /><h3 className="font-black">Membership payment</h3></div>
        <label className="mb-2 block text-sm font-bold text-slate-700">Membership type</label><div className="grid grid-cols-2 gap-3"><button type="button" onClick={() => update("plan", "monthly")} className={`rounded-2xl border p-4 text-left transition ${form.plan === "monthly" ? "border-emerald-400 bg-emerald-50 ring-2 ring-emerald-100" : "border-slate-200 bg-white hover:border-emerald-200"}`}><span className="font-black">Monthly</span><span className="mt-1 block text-xs text-slate-500">Every 30 days</span></button><button type="button" onClick={() => update("plan", "weekly")} className={`rounded-2xl border p-4 text-left transition ${form.plan === "weekly" ? "border-emerald-400 bg-emerald-50 ring-2 ring-emerald-100" : "border-slate-200 bg-white hover:border-emerald-200"}`}><span className="font-black">Weekly</span><span className="mt-1 block text-xs text-slate-500">Every 7 days</span></button></div>
        ...
        <div className="mt-4 rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Next payment</p><p className="mt-1 text-lg font-black text-slate-800">{new Date(`${nextPayment}T00:00:00`).toLocaleDateString()}</p><p className="text-xs text-slate-500">Automatically calculated from the payment date — {form.plan === "weekly" ? "7 days" : "30 days"} later.</p></div>
