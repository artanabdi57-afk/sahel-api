...
import { ArrowRight, Check, Eye, EyeOff, Mail, MapPin, Phone, Store, Dumbbell, GraduationCap, ShieldCheck } from "lucide-react";
...
const COPY = {
  en: {
    login: "Welcome back", signup: "Create your Sahel workspace", loginSub: "Choose your management type and sign in. Sahel will keep you in that workspace.", signupSub: "Choose what you manage and we will personalize your workspace.", email: "Email address", password: "Password", phone: "Phone number", name: "Business or organization name", location: "City / location", choose: "What are you managing?", chooseLogin: "Sign in as", create: "Create workspace", signIn: "Sign in", google: "Continue with Google", or: "or", forgot: "Forgot password?", new: "New to Sahel?", existing: "Already have an account?", switchToCreate: "Create account", secure: "Your information is private and protected.", required: "Please choose your management type and complete the required fields.", invalid: "Phone must be 9 digits and start with 61, 62, or 68.", show: "Show password", hide: "Hide password"
  },
  so: {
    login: "Ku soo dhowow", signup: "Samee goobtaada Sahel", loginSub: "Dooro nooca maamulka oo gal. Sahel wuxuu kugu haynayaa goobtaas.", signupSub: "Doro waxa aad maamusho, Sahel-na goobtaada ayuu kuu habaynayaa.", email: "Email-ka", password: "Erayga sirta ah", phone: "Lambarka telefoonka", name: "Magaca ganacsiga ama hay'adda", location: "Magaalada / goobta", choose: "Maxaad maamulaysaa?", chooseLogin: "U gal sida", create: "Samee goobta", signIn: "Gal", google: "Google ku sii wad", or: "ama", forgot: "Ma ilowday erayga sirta ah?", new: "Sahel ma ku cusub tahay?", existing: "Akoon ma leedahay?", switchToCreate: "Samee akoon", secure: "Xogtaadu waa gaar oo la ilaaliyay.", required: "Fadlan dooro nooca maamulka oo buuxi meelaha loo baahan yahay.", invalid: "Telefoonku waa inuu noqdaa 9 lambar oo ku bilaabma 61, 62, ama 68.", show: "Muuji erayga sirta", hide: "Qari erayga sirta"
  }
};
...
export default function AuthPageV2({ mode }) {
  ...
  async function submit(e) {
    ...
    if (!type) { setError(t.required); return; }
    if (signup && form.phone && !/^(61|62|68)\d{7}$/.test(form.phone.replace(/\D/g, ""))) { setError(t.invalid); return; }
    setLoading(true);
    try {
      const body = signup ? { ...form, phone: form.phone.replace(/\D/g, ""), business_type: type } : { email: form.email, password: form.password, business_type: type };
  ...
  async function google() {
    setError("");
    if (!type) { setError(t.required); return; }
    localStorage.setItem("sahel_google_business_type", type);
  ...
  return <main className="min-h-screen bg-[#f6f8fc] text-slate-900"><div className="mx-auto grid min-h-screen max-w-7xl lg:grid-cols-[1.05fr_.95fr]">
    <aside className="relative hidden overflow-hidden bg-gradient-to-br from-blue-700 via-blue-700 to-indigo-950 p-12 text-white lg:flex lg:flex-col lg:justify-between"><div className="absolute -right-24 -top-24 h-72 w-72 rounded-full bg-white/10 blur-3xl"/><div><img src={sahelLogo} className="h-9 brightness-0 invert" alt="Sahel"/><div className="mt-20 max-w-lg"><p className="text-sm font-bold uppercase tracking-[.18em] text-blue-200">One platform. Your workspace.</p><h1 className="mt-5 text-5xl font-black leading-[1.08]">Run your organization with clarity.</h1><p className="mt-6 text-lg leading-8 text-blue-100">Choose your management type once. Every login and Google sign-in will return you to the same workspace.</p></div></div><div className="grid grid-cols-2 gap-3"><div className="rounded-2xl border border-white/10 bg-white/10 p-4"><ShieldCheck className="mb-3 h-5 w-5 text-blue-200"/><p className="text-sm font-bold">Locked workspace</p></div><div className="rounded-2xl border border-white/10 bg-white/10 p-4"><Check className="mb-3 h-5 w-5 text-blue-200"/><p className="text-sm font-bold">No accidental switching</p></div></div></aside>
    <section className="flex items-center justify-center p-5 sm:p-8"><div className="w-full max-w-xl"><div className="mb-7 flex items-center justify-between"><Link to="/welcome"><img src={sahelLogo} className="h-8 lg:hidden" alt="Sahel"/></Link><div className="ml-auto flex rounded-full bg-white p-1 shadow-sm">{["en", "so"].map(x => <button key={x} onClick={() => setLang(x)} className={`rounded-full px-3 py-1.5 text-xs font-bold ${lang === x ? "bg-blue-600 text-white" : "text-slate-500"}`}>{x.toUpperCase()}</button>)}</div></div>
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_25px_70px_rgba(15,23,42,.08)] sm:p-9"><div className="mb-7"><div className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50"><img src={sahelLogo} className="h-7" alt="Sahel"/></div><h2 className="text-3xl font-black tracking-tight">{signup ? t.signup : t.login}</h2><p className="mt-2 max-w-md text-sm leading-6 text-slate-500">{signup ? t.signupSub : t.loginSub}</p></div>
        <TypeSelector value={type} onChange={setType} title={signup ? t.choose : t.chooseLogin}/>
