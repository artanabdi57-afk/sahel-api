...
export default function Layout() {
  ...
  const isSchool = shop?.business_type === "school";
  ...
  return <div className={`min-h-screen bg-slate-50 ${isSchool ? "school-mobile-shell" : ""}`}>
    <Sidebar />
    ...
    <main className={`min-h-screen transition-[margin] duration-300 lg:pb-0 ${isSchool ? "pb-3" : "pb-24"} ${collapsed ? "lg:ml-20" : "lg:ml-64"}`}>
      ...
      <div className={isDashboard ? `mx-auto max-w-7xl px-3 py-4 sm:px-4 lg:px-8 lg:py-8 ${isSchool ? "school-dashboard-frame" : ""}` : isReports ? "w-full px-4 py-5 lg:px-8 lg:py-7" : "mx-auto max-w-7xl px-4 py-5 lg:px-8 lg:py-7"}><Outlet /></div>
    </main>
    {!isSchool && <nav className="fixed bottom-0 left-0 right-0 z-20 border-t border-slate-200 bg-white/95 px-2 pb-2 pt-1 shadow-lg backdrop-blur-xl lg:hidden"><div className="flex gap-1 overflow-x-auto">{items.map((item) => <NavItem key={item.path} item={item} compact={false} t={t} />)}</div></nav>}
