...
export default function Layout() {
  ...
  if (!shop) return <div className="flex min-h-screen items-center justify-center bg-slate-50"><LoadingState variant="dashboard" /></div>;

  const items = getNavForBusinessType(shop.business_type);
  const isDashboard = location.pathname === "/dashboard" || location.pathname === "/hospital/dashboard";
    ...
    <aside className={[mobile ? "fixed inset-y-0 left-0 z-50 w-[280px] animate-[slideIn_.25s_ease-out]" : `fixed left-0 top-0 z-30 hidden h-screen lg:block ${collapsed ? "w-20" : "w-64"}`, "border-r border-slate-200 bg-white/95 p-4 shadow-xl shadow-slate-200/30 backdrop-blur-xl transition-[width,transform] duration-300 ease-out"].join(" ")}>
      <div className={`mb-7 flex items-center ${collapsed && !mobile ? "justify-center" : "justify-between gap-3"}`}>
        <div className="flex min-w-0 items-center gap-3">
          ...
          {(!collapsed || mobile) && <div className="min-w-0 animate-[fadeIn_.2s_ease-out]"><p className="truncate text-lg font-black text-slate-950">{shopName}</p><p className="truncate text-xs font-semibold capitalize text-slate-400">{shop.business_type?.replace(/_/g, " ") || "Business workspace"}</p></div>}
        </div>
