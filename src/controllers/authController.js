...
const validateBusinessType = (businessType) =>
  !businessType || VALID_BUSINESS_TYPES.includes(businessType);
...
const login = async (req, res, next) => {
  try {
    const { email, phone, identifier, password, business_type } = req.body;
    ...
    if (!authError && authData?.user) {
      ...
      if (!shop) return res.status(403).json({ message: "Workspace setup is required for this account." });
      if (shop.status === "suspended") return res.status(403).json({ message: "This account is suspended." });

      if (business_type && shop.business_type !== business_type) {
        return res.status(403).json({
          message: `This account is registered as ${shop.business_type}. Please choose ${shop.business_type} management to continue.`
        });
      }
    ...
    const { data: staffData, error: staffError } = await withRetry(() =>
      supabaseAdmin.rpc("staff_login", { p_email: normalizedEmail, p_password: password })
    );
    if (staffError || !staffData) return res.status(401).json({ message: "Invalid email or password." });

    const staff = staffData;
    const { data: shop, error: shopError } = await supabaseAdmin
      .from("shops")
      .select("id, shop_name, location, status, plan, business_type")
      .eq("id", staff.shop_id)
      .single();
    if (shopError || !shop) return res.status(403).json({ message: "Workspace not found." });
    if (shop.status === "suspended") return res.status(403).json({ message: "This account is suspended." });
    if (business_type && shop.business_type !== business_type) {
      return res.status(403).json({ message: `This account is registered as ${shop.business_type}. Please choose ${shop.business_type} management to continue.` });
    }

    const token = jwt.sign(
      { user_id: staff.id, email: staff.email, phone: staff.phone || null, shop_id: staff.shop_id, shop_name: shop.shop_name, business_type: shop.business_type || null, role: staff.role || "staff", is_staff: true, onboarding_required: false },
      process.env.JWT_SECRET,
      { expiresIn: "12h" }
    );
    return res.json({ data: { token, user: { id: staff.id, email: staff.email, phone: staff.phone || null, name: staff.name, role: staff.role || "staff" }, shop, is_staff: true } });
  } catch (error) {
    next(error);
  }
};
...
const oauthSession = async (req, res, next) => {
  try {
    const { access_token, business_type } = req.body;
    if (!access_token) return res.status(400).json({ message: "access_token is required." });
    if (!validateBusinessType(business_type)) return res.status(400).json({ message: "Invalid management type." });

    const { data: authData, error: authError } = await withRetry(() => supabase.auth.getUser(access_token));
    if (authError || !authData?.user?.email) return res.status(401).json({ message: "Invalid Google session." });

    const authId = authData.user.id;
    const email = authData.user.email.trim().toLowerCase();
    const shop = await getShopForAuthUser(authId);

    if (shop && business_type && shop.business_type !== business_type) {
      return res.status(403).json({
        message: `This Google account is registered as ${shop.business_type}. Please choose ${shop.business_type} management to continue.`
      });
    }
...
const changePassword = async (req, res, next) => {
  try {
    ...
    const { error } = await supabaseAdmin.auth.admin.updateUserById(req.user.user_id, { password: new_password });
    if (error) throw error;
...
const switchShop = async (req, res, next) => {
  try {
    ...
    const { data: shop, error: shopError } = await supabaseAdmin
      ...
      .eq("id", shop_id).eq("owner_id", authId).single();
    if (shopError || !shop) return res.status(403).json({ message: "Workspace not found or access denied." });
    if (shop.status === "suspended") return res.status(403).json({ message: "This account is suspended." });
...
const staffLogin = async (req, res, next) => {
  try {
    ...
    const { data, error } = await supabaseAdmin.rpc("staff_login", { p_email: email.trim().toLowerCase(), p_password: password });
    if (error || !data) return res.status(401).json({ message: "Invalid email or password." });
    const staff = data;
    const { data: shop, error: shopError } = await supabaseAdmin.from("shops").select("id, shop_name, location, status, plan, business_type").eq("id", staff.shop_id).single();
    if (shopError || !shop) return res.status(403).json({ message: "Workspace not found." });
    if (shop.status === "suspended") return res.status(403).json({ message: "This account is suspended." });
    if (business_type && shop.business_type !== business_type) return res.status(403).json({ message: `This account is registered as ${shop.business_type}. Please choose ${shop.business_type} management to continue.` });
    const token = jwt.sign({ user_id: staff.id, email: staff.email, phone: staff.phone || null, shop_id: staff.shop_id, shop_name: shop.shop_name, business_type: shop.business_type || null, role: staff.role || "staff", is_staff: true, onboarding_required: false }, process.env.JWT_SECRET, { expiresIn: "12h" });
    res.json({ data: { token, user: { id: staff.id, email: staff.email, phone: staff.phone || null, name: staff.name, role: staff.role || "staff" }, shop, is_staff: true } });
  } catch (error) { next(error); }
};
