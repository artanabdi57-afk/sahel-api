...
async function getManagementDashboard(req, res, next) {
  try {
    ...
    const [staff, doctors, appointments, medicines, labs, bills, expenses, todayBills, todayExpenses] = await Promise.all([
      supabase.from("hospital_staff").select("id,full_name,role,active,hospital_departments(id,name)").eq("shop_id", shopId).eq("active", true).order("full_name"),
      supabase.from("hospital_staff").select("id,full_name,role,active,hospital_departments(id,name)").eq("shop_id", shopId).eq("active", true).ilike("role", "%doctor%").order("full_name"),
    ...
    for (const result of [staff, doctors, appointments, medicines, labs, bills, expenses, todayBills, todayExpenses]) {
      if (result.error) throw result.error;
    }

    const medicineRows = medicines.data || [];
    const expenseRows = expenses.data || [];
    const todayExpenseRows = todayExpenses.data || [];
    const todayBillRows = todayBills.data || [];
    const lowStock = medicineRows.filter((item) => Number(item.quantity || 0) <= Number(item.reorder_level || 0));
    const sum = (rows, key) => rows.reduce((total, row) => total + Number(row[key] || 0), 0);
    const salaryTotal = expenseRows.filter((e) => String(e.category || "").toLowerCase() === "salary");
    const todaySalaries = todayExpenseRows.filter((e) => String(e.category || "").toLowerCase() === "salary");

    res.json({ data: {
      staff: staff.data || [],
      doctors: doctors.data || [],
      today_appointments: appointments.data || [],
      medicines: medicineRows,
      low_stock: lowStock,
      lab_requests: labs.data || [],
      pending_labs: (labs.data || []).filter((item) => item.status === "pending").length,
      revenue_today: sum(todayBillRows, "paid"),
      revenue_total: sum(bills.data || [], "paid"),
      expenses_today: sum(todayExpenseRows, "amount"),
      expenses_total: sum(expenseRows, "amount"),
      salaries_today: sum(todaySalaries, "amount"),
      salaries_total: sum(salaryTotal, "amount"),
      outstanding: (bills.data || []).reduce((total, row) => total + Math.max(0, Number(row.total || 0) - Number(row.paid || 0)), 0),
      appointment_count_today: (appointments.data || []).length,
      active_staff_count: (staff.data || []).length,
      doctor_count: (doctors.data || []).length,
    } });
  } catch (error) {
    next(error);
  }
