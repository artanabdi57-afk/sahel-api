...
async function getManagementDashboard(req, res, next) {
  try {
    const shopId = req.user.shop_id; const start = new Date(); start.setHours(0, 0, 0, 0); const end = new Date(start); end.setDate(end.getDate() + 1);
    ...
    const lowStock = medicineRows.filter((item) => Number(item.quantity || 0) <= Number(item.reorder_level || 0)); const sum = (rows, key) => rows.reduce((total, row) => total + Number(row[key] || 0), 0);
    const salaries = expenseRows.filter((e) => String(e.category || "").toLowerCase() === "salary"), todaySalaries = todayExpenseRows.filter((e) => String(e.category || "").toLowerCase() === "salary");
