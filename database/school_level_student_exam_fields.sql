-- School schema additions used by the School Classes, Students and Exams pages.
-- Safe to run on existing Sahel installations.
...
create unique index if not exists school_students_shop_registration_unique_idx
  on public.school_students(shop_id, registration_no)
  where registration_no is not null;

create or replace function public.school_student_registration_no()
returns trigger language plpgsql as $$
declare
  next_no integer;
begin
  if new.registration_no is null or btrim(new.registration_no) = '' then
    select coalesce(max((regexp_match(registration_no, '^STU-([0-9]+)$'))[1]::integer), 0) + 1
      into next_no
...
create trigger school_students_registration_no
before insert on public.school_students
for each row execute function public.school_student_registration_no();
